...
def ktest(df):
    ...
    plot.update_layout(legend_title = "Clust. Alg")
    plot.update_xaxes(title = "n cluster")
    plot.update_yaxes(title = "Silhouette coeffcient" )
    plot.show()
    print(f"Best Silhouette coefficient is {results[0].max()}\n")
    print(f"With {results[results[0] == results[0].max()].index.values} clusters")
    return results
